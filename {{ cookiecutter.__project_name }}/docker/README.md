...
a docker containers for _{{ cookiecutter.__project_name }}_!
...
```bash
make docker-gen IMAGE={{ cookiecutter.__project_name }} VERSION=latest
```

By default, the values for these environment variables are; <br>

```bash
IMAGE := {{ cookiecutter.__project_name }}
VERSION := latest
```
...
```bash
docker run {{ cookiecutter.__project_name }}
```
...
```bash
make clean-docker IMAGE={{ cookiecutter.__project_name }} VERSION=latest
```

## Debug Images

The [dockerfile](./Dockerfile) for _{{ cookiecutter.__project_name }}_ uses
...
```bash
make docker-debug IMAGE={{ cookiecutter.__project_name }}-debug VERSION=latest
```
...
```bash
docker run {{ cookiecutter.__project_name }}-debug
```

You can run a debug image, and directly `shell` into it with

```bash
docker run --rm -it {{ cookiecutter.__project_name }}-debug bash
```
...
```bash
make clean-docker IMAGE={{ cookiecutter.__project_name }}-debug VERSION=latest
```
