...
| `run` | Run _{{ cookiecutter.__project_name.strip() }}_ | NA |
| `docker-gen` | Create production docker image for _{{ cookiecutter.__project_name.strip() }}_ | docker |
| `docker-debug` | Create debug-friendly docker image for _{{ cookiecutter.__project_name.strip() }}_ | docker |
...
```text
{{ cookiecutter.__project_name }}_<version>_<target-os>_<architecture>.<extension>
```
...
```sh
{{ cookiecutter.__project_name }}_v1.2.1_windows_x86_64.exe
```
...
To run `{{ cookiecutter.__project_name }}` in a docker container, read the instructions in
