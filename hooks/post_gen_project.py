...
# Contains path to the project directory - i.e. {{cookiecutter.__project_name}}
...
def remove_codecov(*, force: bool = False):
    ...
    codecov_needed: bool = bool(
        "{{ cookiecutter.use_codecov }}".lower() == "y")
...
def remove_precommit():
    ...
    use_precommit: bool = bool(
        "{{ cookiecutter.use_precommit }}".lower() == "y")
    if use_precommit:
        return

    config_file: str = os.path.join(CUR_DIR, ".pre-commit-config.yaml")
    workflow: str = os.path.join(
        CUR_DIR, ".github", "workflows", "pre-commit.yml")
...
def remove_devcontainer():
    """
    If devcontainer is not used, remove the .devcontainer folder
    """
    use_devcontainer: bool = bool(
        "{{ cookiecutter.use_devcontainer }}".lower() == "y")
    if use_devcontainer:
        return

    dest_path: str = os.path.join(CUR_DIR, ".devcontainer")
    if not os.path.exists(dest_path):
        return False  # The directory to delete does not exist
...
def print_final_instructions():
    ...
    Your project `{{ cookiecutter.__project_name }}` is ready!
    ...
    1) Move to project directory, and initialize a git repository:
        $ cd {{ cookiecutter.__project_name }} && git init
